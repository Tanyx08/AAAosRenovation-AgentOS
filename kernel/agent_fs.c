...
//
// 这个文件集中实现任务四的全部核心能力：
// 真实 inode 上的 Agent 元数据维护、内存中文件元数据缓存、
// 属性倒排索引、共享查询缓存，以及 `query_file` 等文件工具调用。
...
static int file_attr_match(struct inode_attr *attrs, int attr_count,
                           const char *key, const char *value);
static void file_index_reset_locked(void);
static int file_add_posting_locked(int entry_idx, const char *key,
                                   const char *value);
static int file_cache_find_by_inum_locked(uint inum);
static void file_cache_fill_from_inode_locked(struct agent_file_meta *meta,
                                              struct inode *ip,
                                              const char *path);
static int file_cache_upsert_locked(struct inode *ip, const char *path);
static void file_rebuild_postings_locked(void);
static int file_join_path(const char *base, const char *name, char *out,
                          int outsz);
...
static int file_query_parse(const char *params,
                            struct agent_file_query_cond *conds,
                            int *cond_count, char *keyword, int keyword_sz,
                            int *force_scan);
static int file_meta_matches(struct agent_file_meta *meta,
                             struct agent_file_query_cond *conds,
                             int cond_count, const char *keyword);
static void append_file_result(char **ptr, int *left,
                               struct agent_file_meta *meta);
static int query_allows_public_share(const char *params);
static int cache_access_allowed(struct proc *p,
                                struct shared_query_cache *entry);
static void query_result_with_cache(char *dst, int dstsz, const char *base,
                                    int hit, int owner_pid, int refcnt,
                                    uint64 version, int fs_scanned);
static int shared_query_cache_lookup(struct proc *p, const char *query,
                                     struct agent_tool_response *resp);
static void shared_query_cache_store(struct proc *p, const char *query,
                                     const char *result);
static void agent_file_version_bump(void);
static void inode_summary_refresh(struct inode *ip);

static void
agent_fs_runtime_init(void)
{
  if(agent_fs_runtime_ready)
    return;
...
file_add_posting_locked(int entry_idx, const char *key, const char *value)
{
  ...
  safestrcpy(file_postings[slot].value, value,
             sizeof(file_postings[slot].value));
...
void
agentfs_tool_get_file_attr(struct agent_tool_request *req,
                           struct agent_tool_response *resp)
{
  char path[AGENT_FILE_PATH_MAX], key[INODE_ATTR_KEY_MAX];
  char *buf;
...
void
agentfs_tool_del_file_attr(struct agent_tool_request *req,
                           struct agent_tool_response *resp)
{
  char path[AGENT_FILE_PATH_MAX], key[INODE_ATTR_KEY_MAX];
  struct inode *ip;
...
void
agentfs_tool_query_file(struct proc *p, struct agent_tool_request *req,
                        struct agent_tool_response *resp)
{
