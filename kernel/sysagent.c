...
{
  int continue_loop, timeout_ticks;
  uint64 uevent;
  argint(0, &continue_loop);
...
{
  uint64 ubatch;
  struct agent_tool_batch_request breq;
  int ret;
  ...
  if(copyout(p->pagetable, ubatch, (char*)bresp, sizeof(*bresp)) < 0)
    ret = -1;
  kfree((void*)bresp);
  return ret;
}
...
{
  uint64 uname, uschema;
  char name[AGENT_TOOL_NAME_MAX];
  struct agent_tool_schema schema;
  struct proc *p = myproc();
  argaddr(0, &uname);
  argaddr(1, &uschema);
  if(copyinstr(p->pagetable, name, uname, sizeof(name)) < 0)
    return -1;
  if(agent_tool_schema_get(p, name, &schema) < 0)
    return -1;
  if(copyout(p->pagetable, uschema, (char*)&schema, sizeof(schema)) < 0)
    return -1;
  return 0;
}
...
sys_agent_lease_begin(void)
{
  ...
  while(n > 0 && off < (int)sizeof(result)-1)
    result[off++] = tmp[--n];
  if(off < (int)sizeof(result)-1){
    result[off++] = ' ';
    result[off] = 0;
  }
  ...
  while(n > 0 && off < (int)sizeof(result)-1)
    result[off++] = tmp[--n];
  result[off] = 0;
  if(copyout(p->pagetable, uresult, result, strlen(result) + 1) < 0)
    return -1;
  return ret;
}
