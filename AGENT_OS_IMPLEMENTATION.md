...
AgentFS 是 xv6 原有文件系统上的运行时元数据层，没有修改磁盘 inode 格式。
...
属性索引:
  hash(key=value) -> bucket linked list

结构化查询:
  query_file 返回 path、属性、summary、count、used_index、index_scanned、full_scanned
```

设置属性时，内核先通过 `namei()` 校验路径存在且目标是普通文件，然后记录属性并刷新摘要：
...
```c
n = readi(ip, 0, (uint64)meta->summary, 0, sizeof(meta->summary) - 1);
meta->summary[n] = 0;
```
...
```text
1. 文件元数据使用第一个属性加入哈希桶
2. query_file 有属性条件时，使用第一个查询条件定位桶
3. 再检查剩余属性条件和 keyword
4. 返回 index_scanned 与 full_scanned 供演示性能差异
```
...
这满足任务四“按属性查询文件、内容摘要索引、属性索引、结构化返回与缓存”的主要验收点。当前属性表是内存表，重启后不会持久化。
...
query_file 的索引策略只选择第一个查询条件对应的哈希桶
内容检索是摘要子串匹配，不是 embedding 或语义搜索
消息事件使用单消息槽，不是完整消息队列
事件源只实现消息事件，尚未加入文件修改等事件
多 Agent 调度沿用 xv6 调度器，尚未实现 Agent 优先级
真实 LLM 建议运行在宿主机，通过串口桥接到 xv6 用户态 Agent 程序
```
